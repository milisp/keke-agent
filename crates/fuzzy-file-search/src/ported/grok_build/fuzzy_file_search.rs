// nucleo's `Snapshot::get_item_unchecked` is `unsafe`; ported as-is with its
// original SAFETY comment rather than working around it, since this is a
// straight port of third-party code, not code keke authored. The `expect`s
// and `unwrap`s below are the same call: they guard invariants the upstream
// crate holds (a static override string, a never-poisoned mutex), not
// something keke's own code should be following as a pattern.
#![allow(unsafe_code, clippy::expect_used, clippy::unwrap_used)]

//! Ported from `../grok-build/crates/codegen/xai-fuzzy-file-search/src/lib.rs`.
//! See `THIRD_PARTY_NOTICES.md`. The Unix `RLIMIT_NPROC` re-exec regression
//! test was dropped here: it depends on `xai-tty-utils`, which keke does not
//! vendor.
//!
